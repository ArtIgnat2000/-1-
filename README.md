...
- `index.htm` — главная страница с расписанием (использует `style.css`, `script.js` и `schedule.json`).
- `schedule.json` — данные расписания 2 «В» класса по дням недели (Пн–Пт).
- `meal.htm` — страница питания: только приёмы пищи 2 «В» класса (завтрак 11:00, обед 14:40, полдник 16:00) с вкладками и подсветкой текущего приёма (также использует `style.css` и `script.js`).
- `holidays.htm` — страница с каникулами и календарём на 2026/2027 (также использует `style.css` и `script.js`).
- `style.css` — общий файл стилей для всех страниц: дизайн в стиле iOS 26 «Liquid Glass» (живой градиентный фон, стеклянные карточки, плавающий док, тёмная тема).
- `script.js` — общий JavaScript: загрузка расписания, переключение дней, дата в шапке, подсветка «сегодня» в календаре каникул.
