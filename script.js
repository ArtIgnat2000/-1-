...
/* Клик по периоду каникул — прокрутка к нужному месяцу в календаре */
function initHolidaysJump() {
    var items = document.querySelectorAll('.vac-item[data-target]');
    items.forEach(function (item) {
        var jump = function () {
            var target = document.getElementById(item.getAttribute('data-target'));
            if (!target) return;
            var grid = document.querySelector('.months-grid');
            if (grid) grid.classList.add('flash-target');
            ...
            setTimeout(function () {
                target.classList.remove('flash');
                if (grid) grid.classList.remove('flash-target');
            }, 2200);
        };
